...
import type { CampaignConfig, ContentItem } from "../config/types.js";
...
export class VideoPipeline {
  ...
  async createDraft(campaign: CampaignConfig): Promise<ContentItem> {
    const [idea] = await this.deps.trends.getIdeas(campaign.niche, 1);
    log.info({ campaignId: campaign.id, idea }, "selected content idea");

    const brief = await writeContentBrief(this.deps.llm, campaign, idea);
  ...
  async renderAssets(item: ContentItem, campaign: CampaignConfig): Promise<ContentItem> {
    const videoPrompt = buildVideoPrompt(item);
...
function buildVideoPrompt(item: ContentItem): string {
  return [
