 ...
 * It is NEVER written to disk or a database. We keep only the analysis result
 * we hand back to the user (and even that is not persisted here).
 *
 * This is NOT legal advice and Law & Armor is not a law firm.
 */
export async function POST(request: NextRequest) {
  try {
    const rl = rateLimited(request, "lawarmor-analyze", 30, 60_000); if (rl) return rl;
    ...
    const analysis = analyzeDocument({ declaredType, fileName, stateCode, textHint });
    ...
    console.error("lawarmor analyze error", e);
