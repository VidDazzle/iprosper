...
import { listAllPartners, listLeads, partnerRevenue } from "@/lib/partners/store";
...
export default async function AdminPartnersPage() {
  const [partners, leads, rev] = await Promise.all([listAllPartners(), listLeads(), partnerRevenue()]);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Attorney Advertising</h1>
        <p className="text-sm text-slate-400">Beacon&rsquo;s partner network and advertising revenue. Flat fees only — no fee-splitting.</p>
      </div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Monthly recurring" value={USD.format(rev.mrr)} sub={`${rev.activePartners} active advertisers`} accent="emerald" />
        ...
        <StatCard label="Total advertisers" value={String(rev.totalPartners)} sub={`${rev.activePartners} active`} accent="amber" />
      </div>
