...
import { UploadCloud, Loader2, ShieldCheck, Lightbulb, AlertTriangle, Scale, ListChecks, HelpCircle, Trash2 } from "lucide-react";

const DOC_TYPES = [
  ["medical_bill", "Medical bill / statement"],
...
interface Analysis {
  ...
  keyPoints: string[]; watchOuts: string[]; rights: string[]; nextSteps: string[]; questions: string[]; stateNote: string;
...
export default function HealthAnalyzer() {
  ...
  async function run(e: React.FormEvent) {
    ...
    try {
      ...
      fd.append("note", note);
      const res = await fetch("/api/health/analyze", { method: "POST", body: fd });
    ...
    <div className="space-y-6">
      <form onSubmit={run} className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        <label className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border border-dashed border-white/20 bg-[#03040a] px-6 py-8 text-center transition-colors hover:border-cyan-400/40">
          ...
          <span className="text-sm text-slate-300">{fileName || "Upload your bill, EOB, or letter (PDF or image, up to 15 MB)"}</span>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. bill is higher than my EOB"
              className="h-10 border-white/15 bg-[#03040a] text-white placeholder:text-slate-600" />
          </div>
        </div>
